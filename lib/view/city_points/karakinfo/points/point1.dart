import 'package:customer_transpoition/MC/controller/controller/karak.dart';
import 'package:customer_transpoition/MC/controller/model/karak.dart';
import 'package:customer_transpoition/widget/future/future1.dart';

import 'package:flutter/material.dart';

class karakpoint1 extends StatefulWidget {
  const karakpoint1({super.key});

  @override
  _karakpoint1 createState() => _karakpoint1();
}

class _karakpoint1 extends State<karakpoint1> {
  late Future<List<karak>> tasks;
  List data = [];

  final karakInfoData _karakInfo = karakInfoData();
  void initState() {
    super.initState();
    tasks = _karakInfo.fetchdata();
  }

  @override
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          title: Text('مجمع الكرك الداخلي '),
        ),
        body: future1(tasks: tasks, data: data, i: 0));
  }
}
